...
results_path = "wbase_results.txt";
...
if (os.path.isfile(results_path)): #load results from file
    fresults = open(results_path,"r");
    for line in fresults:
        ...
        words = tok[1].split(',');
        for w in words:
            results[ty][tx].append(w.strip());
    fresults.close();
else:               #construct results and write to file
    fresults = open(results_path,"w");
    for sx in range(width):
        for sy in range(height):
            print ("sx: " + str(sx) + " sy: " + str(sy));
            word_list = []
            f_words(sx,sy);
            fresults.write(str(sx)+","+str(sy)+":");
            for i in range(len(word_list)):
                results[sy][sx].append(word_list[i]);
                if (i<(len(word_list)-1)):
                    fresults.write(word_list[i] + ", ");
                else:
                    fresults.write(word_list[i] + "\n");  
    fresults.close();    
